...
(function ($) {
    ...
    _easyform.prototype = {
        init: function () {
            ...
            //改写 submit 的属性，便于控制
            $this.form.find("input:submit").each(function () {
                var button = $(this);
        ...
        _load: function (iterator) {
            ...
            if (!iterator) {
                iterator = "input[data-easyform]:visible,input[data-message]:visible,textarea[data-easyform]:visible,textarea[data-message]:visible";
            }
                ...
                if (input.type != "hidden" && input.type != "button" && input.type != "submit"
                    ...
                    if (input.type == "radio" || input.type == "checkbox") {
                        ...
                        for (index in $this.inputs) {
                            if ($this.inputs.hasOwnProperty(index) && name == $this.inputs[index].input[0].name) {
                                return;
                            }
                        }
                    }
                    ...
                    checker.complete = function (e) {
                        //记录已经完成检查的input
                        $this._array_add_unique($this.counter, e.id || e.name);
                    ...
                    checker.success = function (e) {
                        //记录检查成功的控件
                        $this._array_add_unique($this.counter_success, e.id || e.name);
...
(function ($) {
    ...
    var _easyinput = function (input, opt) {
        ...
        o.easytip = $this.rules.easytip;
        o['real-time'] = $this.rules['real-time'];
    ...
    _easyinput.prototype = {
        ...
        _null: function (ei, v, r, o) {
            var keys = Object.keys(r || {}).length;

            if (!v) {
                // 当前值为空
                //rule不为空并且含有null， 或rule为空
                if (keys > 0 && typeof(r["null"]) != "undefined") {
                    if (false == o.realtime) {
...
(function ($, window, document) {
    var _easytip = function (ele, opt) {
        ...
        this.id = "easytip-div-main-" + (ele[0].id || ele[0].name);
    };
    ...
    _easytip.prototype = {
        ...
        init: function () {
            ...
            //同一个控件不会多次初始化。
            if (tip.length == 0) {
                $(document.body).append("<div id=\"" + id + "\"><div class=\"easytip-text\"></div><div class=\"easytip-arrow\"></div></div>");

                tip = $("#" + id);
                var text = $("#" + id + " .easytip-text");
        ...
        _position: function () {
            ...
            tip.css({
                left: parseInt(this.options.left) + left,
                top: parseInt(this.options.top) + top
            });
        },
