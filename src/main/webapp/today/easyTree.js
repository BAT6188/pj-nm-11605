...
(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            addable: false,
            i18n: {
                deleteNull: '请选择一个删除节点',
                deleteConfirmation: '确定删除?',
                confirmButtonLabel: '确定',
                editNull: '请选择一个编辑节点',
                editMultiple: '每次只能编辑一个节点',
                addMultiple: '请选择添加的父节点',
        ...
        var createInput = $('<div><input type="text" placeholder="请输入列表名" class="form-control"><span class="input-group-btn"><button type="button" class="btn btn-default btn-success confirm">确定</button> </span><span class="input-group-btn"><button type="button" class="btn btn-default cancel">取消</button> </span> </div> ');
           ...
           /* if (options.deletable || options.editable || options.addable) {
            ...
            /*if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createBlock).before(createInput);
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        if (selected.length <= 0) {
                            ...
                            if ($(selected).hasClass('parent_li')) {
                                ...
                                $(selected).addClass('parent_li').find(' > span > span').addClass('fa-folder-open').removeClass('fa-file');
            ...
            // deletable
            if (options.deletable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="remove"><button class="btn btn-default btn-sm btn-danger disabled"><span class="fa fa-times"></span></button></div> ');
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            ...
                            if($(selected).parent('ul').find(' > li').length <= 1) {
                                $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass('fa-folder-open').addClass('fa-file');
